...
public class SeatMapView {
    ...
    public void render(String sectionId, String sectionName) {
        ...
        for (String[] row : grid) java.util.Arrays.fill(row, "   "); // ô trống
        for (Seat s : seats) {
        ...
        System.out.print("     ");
        for (int c = 1; c <= displayCols; c++) System.out.printf(" %02d ", c);
        if (maxCol > displayCols) System.out.print(" ...");
        System.out.println();
    ...
    public void renderAvailableList(String sectionId, int limit) {
        ...
        available.stream().limit(limit).forEach(s ->
            System.out.printf("  %-12s (Hang %02d, Ghe %02d)%n",
